...
describe("MailBuilder", () => {
	...
	it("build() without from throws EmailValidationError", () => {
		expect(() => new MailBuilder().to("c@d.com").subject("Hi").text("body").build()).toThrow(
			EmailValidationError,
		)
	})

	it("build() without to throws EmailValidationError", () => {
		expect(() => new MailBuilder().from("a@b.com").subject("Hi").text("body").build()).toThrow(
			EmailValidationError,
		)
	})

	it("build() without subject throws EmailValidationError", () => {
		expect(() => new MailBuilder().from("a@b.com").to("c@d.com").text("body").build()).toThrow(
			EmailValidationError,
		)
	})

	it("build() without text or html throws EmailValidationError", () => {
		expect(() => new MailBuilder().from("a@b.com").to("c@d.com").subject("S").build()).toThrow(
			EmailValidationError,
		)
	})

	it("multiple to() recipients via varargs", () => {
		const opts = new MailBuilder()
			.from("a@b.com")
			.to("a@a.com", "b@b.com")
			.subject("Hi")
			.text("body")
