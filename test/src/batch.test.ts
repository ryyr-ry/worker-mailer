...
function makeDelayMailer(responses: Array<{ result: SendResult | Error; delay: number }>): {
	mailer: Mailer
	callOrder: number[]
} {
	let callIndex = 0
...
describe("sendBatch - concurrency edge cases", () => {
	...
	it("mixed success/failure with continueOnError=true (concurrent)", async () => {
		const mailer = makeMockMailer([makeResult("1"), new Error("fail"), makeResult("3")])
		const results = await sendBatch(mailer, [email1, email2, email3], {
			concurrency: 2,
			continueOnError: true,
		})
	...
	it("concurrency=NaN throws ConfigurationError", async () => {
		const { mailer } = makeDelayMailer([{ result: makeResult("1"), delay: 10 }])
		await expect(sendBatch(mailer, [email1], { concurrency: Number.NaN })).rejects.toThrow(
			"concurrency must be a non-negative integer",
		)
	})

	it("concurrency=-1 throws ConfigurationError", async () => {
		const { mailer } = makeDelayMailer([{ result: makeResult("1"), delay: 10 }])
		await expect(sendBatch(mailer, [email1], { concurrency: -1 })).rejects.toThrow(
			"concurrency must be a non-negative integer",
		)
	})

	it("concurrency=1.5 (non-integer) throws ConfigurationError", async () => {
		const { mailer } = makeDelayMailer([{ result: makeResult("1"), delay: 10 }])
		await expect(sendBatch(mailer, [email1], { concurrency: 1.5 })).rejects.toThrow(
			"concurrency must be a non-negative integer",
		)
	})
})
