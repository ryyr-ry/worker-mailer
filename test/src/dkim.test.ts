...
describe("Canonicalization (RFC 6376 Section 3.4)", () => {
	it("relaxed header: lowercases name and collapses whitespace", () => {
		expect(canonicalizeRelaxedHeader("Subject", "  Hello   World ")).toBe("subject:Hello World")
	})

	it("relaxed header: unfolds continuation lines", () => {
		expect(canonicalizeRelaxedHeader("Subject", " line1\r\n\tline2")).toBe("subject:line1 line2")
	})
	...
	it("simple header: preserves original exactly", () => {
		expect(canonicalizeSimpleHeader("Subject", " Hello  World ")).toBe("Subject: Hello  World ")
	})
...
describe("Key management", () => {
	...
	it("importDkimKey rejects empty PEM", async () => {
		await expect(
			importDkimKey("-----BEGIN PRIVATE KEY----------END PRIVATE KEY-----"),
...
describe("signDkim (RFC 6376 Section 3.5)", () => {
	...
	it("DKIM-Signature has required tags v,a,d,s,h,bh,b", async () => {
		...
		const sigLine = signed
			.split("\r\n\r\n")[0]
			.split("\r\n")
			.filter((l) => l.startsWith("DKIM-Signature:") || /^[\t ]/.test(l))
			.join("")
	...
	it("signature is verifiable with public key", async () => {
		...
		for (const h of [
			...
			if (found) {
				...
				signingParts.push(
					canonicalizeRelaxedHeader(found.substring(0, colonIdx), found.substring(colonIdx + 1)),
				)
			}
		}
		...
		signingParts.push(
			canonicalizeRelaxedHeader(
				templateNoB.substring(0, tmplColon),
				templateNoB.substring(tmplColon + 1),
			),
		)
...
describe("Relaxed header canonicalization edge cases", () => {
	it("collapses multiple spaces to single space", () => {
		expect(canonicalizeRelaxedHeader("Subject", "  lots   of   spaces")).toBe(
			"subject:lots of spaces",
		)
	})
...
describe("signDkim() edge cases", () => {
	it("signs message with empty body (headers + blank line only)", async () => {
		...
		const emptyDigest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode("\r\n"))
	...
	it("folds long DKIM-Signature b= value across lines", async () => {
		...
		const headerEnd = signed.indexOf("\r\n\r\n")
		const headerLines = signed.substring(0, headerEnd).split("\r\n")
		const sigLines = headerLines.filter((l) => l.startsWith("DKIM-Signature:") || /^[\t ]/.test(l))
...
describe("DKIM parameter injection prevention", () => {
	it("domainName with CRLF throws DkimError", async () => {
		await expect(signDkim(MSG, { ...opts, domainName: "evil.com\r\ninjected" })).rejects.toThrow(
			DkimError,
		)
	})

	it("domainName with whitespace throws DkimError", async () => {
		await expect(signDkim(MSG, { ...opts, domainName: "evil .com" })).rejects.toThrow(DkimError)
	})

	it("domainName with semicolon throws DkimError", async () => {
		await expect(signDkim(MSG, { ...opts, domainName: "evil;com" })).rejects.toThrow(DkimError)
	})

	it("keySelector with CRLF throws DkimError", async () => {
		await expect(signDkim(MSG, { ...opts, keySelector: "test\r\ninjected" })).rejects.toThrow(
			DkimError,
		)
	})

	it("keySelector with whitespace throws DkimError", async () => {
		await expect(signDkim(MSG, { ...opts, keySelector: "test selector" })).rejects.toThrow(
			DkimError,
		)
	})

	it("keySelector with semicolon throws DkimError", async () => {
		await expect(signDkim(MSG, { ...opts, keySelector: "test;bad" })).rejects.toThrow(DkimError)
	})

	it("From header must be included in signed headers (RFC 6376)", async () => {
		await expect(signDkim(MSG, { ...opts, headerFieldNames: ["to", "subject"] })).rejects.toThrow(
			"from",
		)
	})

	it("From header check is case-insensitive", async () => {
		const signed = await signDkim(MSG, { ...opts, headerFieldNames: ["From", "to"] })
		expect(signed).toContain("DKIM-Signature:")
	})
})
