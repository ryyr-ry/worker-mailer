...
import { ConfigurationError, SmtpCommandError, SmtpConnectionError } from "../../src/errors"
...
describe("WorkerMailer (RFC 5321)", () => {
	...
	it("connection timeout throws SmtpConnectionError", async () => {
		...
		vi.mocked(connect).mockReturnValue(socket as never)
		await expect(WorkerMailer.connect({ ...OPTS, socketTimeoutMs: 50 })).rejects.toThrow(
			SmtpConnectionError,
		)
	})
})
