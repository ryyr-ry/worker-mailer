...
describe("Template engine", () => {
	...
	it("{{var}} HTML-escapes output (XSS prevention)", () => {
		expect(render("{{val}}", { val: "<script>alert(1)</script>" })).toBe(
	...
	it("{{^key}}...{{/key}} renders when falsy (inverted section)", () => {
		expect(render("{{^hidden}}visible{{/hidden}}", { hidden: false })).toBe("visible")
	})

	it("{{#array}}...{{/array}} iterates over items", () => {
		expect(render("{{#items}}{{.}},{{/items}}", { items: ["a", "b"] })).toBe("a,b,")
	})
...
describe("Template - object section context", () => {
	it("object section pushes properties into context", () => {
		expect(render("{{#user}}{{name}}{{/user}}", { user: { name: "Bob" } })).toBe("Bob")
	})

	it("object section retains outer scope access", () => {
		expect(
			render("{{#user}}{{name}} ({{role}}){{/user}}", { user: { name: "Bob" }, role: "admin" }),
		).toBe("Bob (admin)")
	})

	it("inner property shadows outer property", () => {
		expect(render("{{#user}}{{name}}{{/user}}", { user: { name: "Inner" }, name: "Outer" })).toBe(
			"Inner",
		)
	})

	it("nested objects push context recursively", () => {
		expect(render("{{#a}}{{#b}}{{val}}{{/b}}{{/a}}", { a: { b: { val: "deep" } } })).toBe("deep")
	})
})
