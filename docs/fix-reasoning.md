...
### #10 worker-mailer.ts 262行→250行以下 (P2)

**問題の本質:**
worker-mailer.tsが267行（フェーズ2修正後）で250行制限を超過。

**修正方針:**
大規模な構造変更は避け、以下の小さな圧縮を組み合わせて17行削減:
1. constructorの認証情報バリデーションを圧縮（-2行）
2. authenticate()呼び出しの引数を1行に統合（-4行）
3. tryReconnect()のmaxAttempts/msg変数をインライン化（-2行）
4. close()のsendingPromise待機を1行に（-2行）
5. processEmailWithRetry()のmax-retries処理を圧縮（-3行）
6. reportFatalError()を1行化（-1行）
7. send()のhookResult分岐を圧縮（-1行）
8. ping()を条件式で1行化（-1行）
9. 必要に応じて空行を1つ削除（-1行）

### #16 executeSend 30行超過 → フェーズ2修正で27行に解消済み
### #17 handleSendFailure 30行超過 → フェーズ2修正で26行に解消済み
